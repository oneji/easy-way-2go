...
@section('content')
    ...
    <div class="col-12">
        <div class="card">
            <form method="POST" action="{{ route('admin.helpItems.update', [$item->id]) }}">
                @csrf
                @method('PUT')

                <div class="card-body">

                    <h4 class="card-title">{{ __('pages.help.createFormLabel') }}</h4>
                    ...
                    <div class="tab-content p-3 text-muted">
                            ...
                            <div class="tab-pane {{ $idx === 0 ? 'active' : null }}" id="{{ $lang->code }}" role="tabpanel">
                                <p class="mb-0">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea name="description[{{ $lang->code }}]" cols="30" rows="5" class="form-control summernote" placeholder="{{ __('form.placeholders.description') }}">{{ $item->getTranslation('description', $lang->code) }}</textarea>
                                    </div>
                                </p>
                            </div>
                        @endforeach
                        ...
                        <div class="form-group">
                            <label>{{ __('form.labels.section') }}</label>
                            <select class="form-control" required name="help_section_id">
                                @foreach ($sections as $section)
                                    <option value="{{ $section->id }}" {{ $item->help_section_id === $section->id ? 'selected' : null }}>{{ $section->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
